...
import { Search, Menu, X, User as UserIcon, LogOut, Crown, ChevronDown } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useAuth } from "../../../../context/AuthContext";

const SUPER_ADMIN_LINKS = ["Users", "Roles", "Permissions", "Settings", "Audit"];
      ...
      <div className="h-16 bg-neutral-950 text-white">
        <div className="max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-semibold tracking-tight">StyleHive Super Admin</span>
          </div>
          ...
          <div className="md:hidden bg-neutral-950 border-t border-neutral-800 px-4 sm:px-6 py-4 flex flex-col gap-2 animate-in slide-down-from-top">
            {SUPER_ADMIN_LINKS.map((link) => (
              <a
                key={link}
                href="#super-admin"
                className="text-sm font-medium text-neutral-300 hover:text-white py-2 px-3 rounded-lg hover:bg-neutral-800 transition-colors duration-200"
            ...
            {user ? (
              <>
                <div className="flex items-center gap-2 py-2 px-3 rounded-lg bg-neutral-800">
                  ...
                  <div>
                    <p className="text-sm font-medium text-neutral-300">{user?.name || "Super Admin"}</p>
                    <p className="text-xs text-neutral-400">{user?.email || ""}</p>
                  </div>
                </div>
                <button
