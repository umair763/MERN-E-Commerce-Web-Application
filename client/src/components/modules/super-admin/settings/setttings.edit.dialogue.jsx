...
export const SettingsEditDialogue = ({
  isOpen,
  setting,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${BaseUrl}/api/settings/${setting.key}`, {
        method: "PUT",
        credentials: "include",
      ...
      if (response.ok) {
        ...
        error(result.message || "Failed to update setting");
        setErrors({ general: result.message });
      }
    } catch (err) {
      error("Failed to update setting");
      setErrors({ general: "Failed to update setting" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-lg">
        <div className="w-full overflow-hidden rounded-xl bg-white font-sans">
          <div className="flex items-center justify-between rounded-t-xl bg-[#4F30A9] px-6 py-4">
            <h2 className="m-0 text-[15px] font-semibold text-white">Edit Setting</h2>
          </div>
            ...
            <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  value={setting?.key || ""}
                  disabled
                  className="rounded-lg border border-gray-300 bg-gray-100 px-4 py-2 text-sm outline-none"
                />
              </div>
