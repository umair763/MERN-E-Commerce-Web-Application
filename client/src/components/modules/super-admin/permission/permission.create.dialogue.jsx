...
export const PermissionCreateDialogue = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        error(result.message || "Failed to create permission");
        setErrors({ general: result.message });
      }
    } catch (err) {
      error("Failed to create permission");
      setErrors({ general: "Failed to create permission" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-lg">
        <div className="w-full overflow-hidden rounded-xl bg-white font-sans">
          <div className="flex items-center justify-between rounded-t-xl bg-[#4F30A9] px-6 py-4">
            <h2 className="m-0 text-[15px] font-semibold text-white">Create Permission</h2>
          </div>
